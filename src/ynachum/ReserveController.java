...
public class ReserveController extends SolitaireReleasedAdapter {
	 ...
	 * @param res The controller's reserve pile
	 */
	public ReserveController(Nestor theGame, PileView res) {
		super(theGame);
		this.res = res;
	}
	...
	public void mousePressed(MouseEvent me) {
		...
		Pile sourceReserve = (Pile) res.getModelElement();
		if (sourceReserve.count() == 0) {
		...
		CardView cardView = res.getCardViewForTopCard (me);
	...
	public void mouseReleased(MouseEvent me) {
		...
		Pile targetReserve = (Pile) res.getModelElement();

		// if the source is a column then perform a column to pile move
		if(fromWidget.getModelElement().getName().contains("column")){
			...
			ColumnToPileMove move = new ColumnToPileMove(sourceColumn, sourceCard, targetReserve);
		...
		// if the source is another pile then perform a pile to pile move
		else{
			Pile sourceReserve = (Pile) fromWidget.getModelElement();
			Card sourceCard = (Card) w.getModelElement();
			
			PileToPileMove move = new PileToPileMove(sourceReserve, sourceCard, targetReserve);
			if(move.doMove(theGame)){
				theGame.pushMove(move);
				theGame.refreshWidgets();
			}
